...
define(['require', 'jquery', 'lodash', 'log', 'alerts'],
    ...
    function (require, $, _, log, Alerts) {
        ...
        var AvroMapper = function (type, container, extensionConfig, mapperData) {
        ...
        AvroMapper.prototype.constructor = AvroMapper;

        AvroMapper.prototype.render = function () {
            var self = this;
            ...
            if (config.customEnabled) {
                ...
                if (this.__mapperType === 'sink') {
                    ...
                    container.find('.custom-mapping-section .payload-field')
                        .on('keyup', function (evt) {
                            config.payload = $(evt.currentTarget).val();
                        });
                } else {
                    extensionConfig.stream.attributes.forEach(function (attrib, i) {
                        config.attributes[attrib.name] = "";
                        container.find('.custom-mapping-section>.attrib-section').append(`
                            <div style="width: 100%; padding-bottom: 5px" class="attribute-map">
